...
Rscript script_write_matrix_cat.R $calls 
...
echo "moves.append( mvScale(mu1,lambda=1, weight=1.0) )" >> mcmc_primates_tmp.Rev
...
echo "ss = steppingStoneSampler(file=\"output_cat_$calls/mk_$calls.$mod.ss\", powerColumnName=\"power\", likelihoodColumnName=\"likelihood\")" >> mcmc_primates_tmp.Rev 
