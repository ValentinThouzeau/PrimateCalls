...
Rscript script_write_matrix_template.R $mod $calls
...
for i in $p
do
	echo "mu$i ~ dnExponential( "$pp" )" >> mcmc_primates_tmp.Rev
    echo "mu$i.setValue( 0.01 )" >> mcmc_primates_tmp.Rev
done
...
echo "pow_p = powerPosterior(mymodel, moves, monitors, \"output_m0_$calls/mk_$calls.$mod.ss\", cats=49)" >> mcmc_primates_tmp.Rev
...
echo "ss = steppingStoneSampler(file=\"output_m0_$calls/mk_$calls.$mod.ss\", powerColumnName=\"power\", likelihoodColumnName=\"likelihood\")" >> mcmc_primates_tmp.Rev 
echo "ss_res = ss.marginal()" >> mcmc_primates_tmp.Rev
echo "write(ss_res, filename = \"ss_res_m0_$calls\")" >> mcmc_primates_tmp.Rev
