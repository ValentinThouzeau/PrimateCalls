...
Rscript script_write_matrix_exp_6.R $calls $mod
...
for i in $p
do
	echo "mu$i ~ dnExponential( 10 )" >> mcmc_primates_tmp.Rev
...
echo "monitors.append( mnModel(filename=\"output_fuller_$calls/mk_$calls.log\", printgen=1) )" >> mcmc_primates_tmp.Rev
