...
Rscript script_write_matrix_hierarchy.R $calls 
...
echo "pow_p = powerPosterior(mymodel, moves, monitors, \"output_hierarchy_$calls/mk_$calls.$mod.ss\", cats=49)" >> mcmc_primates_tmp.Rev
...
echo "ss = steppingStoneSampler(file=\"output_hierarchy_$calls/mk_$calls.$mod.ss\", powerColumnName=\"power\", likelihoodColumnName=\"likelihood\")" >> mcmc_primates_tmp.Rev 
